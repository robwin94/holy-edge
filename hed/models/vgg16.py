...
class Vgg16():
    ...
    def __init__(self, cfgs, run='training'):
        ...
        weights_file = self.cfgs['model_weights_path']

        print('weights: ', weights_file)
        self.data_dict = np.load(weights_file, encoding='latin1').item()
        self.io.print_info("Model weights loaded from {}"
                           .format(self.cfgs['model_weights_path']))
    ...
    def deconv_layer(self, x, upscale, name, padding='SAME', w_init=None):
        ...
        out_shape = tf.stack([x_shape[0], x_shape[1], x_shape[2],
                              w_shape[2]]) * tf.constant(strides, tf.int32)
        deconv = tf.nn.conv2d_transpose(x, W, out_shape, strides=strides,
                                        padding=padding)
    ...
    def get_conv_filter(self, name):
        ...
        # alle filter zufällig initialisiert
        if name == 'conv1_1':
            filt = np.float32(np.random.uniform(-1, 1, (3, 3, 4, 64)))
        filt = np.float32(np.random.uniform(-1, 1, shape))
        return tf.constant(filt, name='filter')

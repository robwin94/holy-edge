...
class IO():
    ...
    def print_warning(self, warning_string):
        ...
        warning = '[{0}][WARNING] {1}'.format(self.get_local_time(),
                                              warning_string)
    ...
    def read_file_list(self, filelist):
        ...
        filenames = [f.split(' ') for f in filenames]
        filenames = [(f[0].strip(), f[1].strip()) for f in filenames]

        return filenames

    def read_file_list_not_split(self, filelist):
    ...
    def split_pair_names(self, filenames, base_dir):
        ...
        filenames = [(os.path.join(base_dir, c[0]),
                      os.path.join(base_dir, c[1]),
